  ...
  return (
    <form onSubmit={handleSubmit} className="grid lg:grid-cols-3 gap-10">
      <div className="lg:col-span-2 space-y-8">
        ...
        >
          <h2 className="font-display text-lg font-bold text-stone-800 mb-4">{t("checkout.yourInfo")}</h2>
          <div className="grid sm:grid-cols-2 gap-4">
            <input
              ...
              className="rounded-xl border border-stone-200 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-brand/40"
            />
            <input
              required
        ...
        <motion.section
          ...
          className="rounded-2xl border border-stone-100 bg-white p-6 shadow-sm"
        >
          <h2 className="font-display text-lg font-bold text-stone-800 mb-4">{t("checkout.deliveryLocation")}</h2>
          <LocationPicker
            lat={coords.lat}
            lng={coords.lng}
          ...
          <div className="grid sm:grid-cols-2 gap-4 mt-4">
            ...
            <textarea
              ...
              className="sm:col-span-2 rounded-xl border border-stone-200 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-brand/40"
              rows={2}
            />
          </div>
        </motion.section>
        ...
        <motion.section
          ...
          className="rounded-2xl border border-stone-100 bg-white p-6 shadow-sm"
        >
          <h2 className="font-display text-lg font-bold text-stone-800 mb-4">{t("checkout.paymentMethod")}</h2>
          <div className="grid sm:grid-cols-2 gap-4">
            <PaymentOption
              icon={Banknote}
          ...
          {paymentMethod === "card" && (
            <p className="mt-3 text-xs text-stone-400">{t("checkout.cardNote")}</p>
          )}
          <textarea
            ...
            className="mt-4 w-full rounded-xl border border-stone-200 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-brand/40"
            rows={2}
          />
        </motion.section>
      </div>
      ...
      <motion.aside
        ...
        className="h-fit rounded-2xl border border-stone-100 bg-white p-6 shadow-sm sticky top-24"
      >
        <h2 className="font-display text-lg font-bold text-stone-800 mb-4">{t("checkout.orderSummary")}</h2>
        <div className="space-y-3 max-h-64 overflow-y-auto pr-1 rtl:pr-0 rtl:pl-1">
          {lines.map((l) => (
            <div key={l.menuItemId} className="flex items-center gap-3">
              <div className="relative h-12 w-12 rounded-lg overflow-hidden shrink-0 bg-stone-100">
                <Image src={l.image} alt={l.name} fill unoptimized className="object-cover" />
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-stone-700 truncate">{l.name}</p>
                <p className="text-xs text-stone-400">x{l.qty}</p>
              </div>
              <span className="text-sm font-semibold text-stone-700">{l.price * l.qty} MAD</span>
            </div>
          ))}
        </div>
        <div className="mt-5 space-y-2 border-t pt-4 text-sm">
          ...
          <div className="flex justify-between font-display text-base font-bold text-brand-dark pt-2 border-t">
  ...
  return (
    <button
      ...
      className={`flex items-center gap-3 rounded-xl border-2 p-4 text-sm font-semibold transition-colors ${
        active ? "border-brand bg-brand/5 text-brand-dark" : "border-stone-200 text-stone-500 hover:border-stone-300"
      }`}
    >
